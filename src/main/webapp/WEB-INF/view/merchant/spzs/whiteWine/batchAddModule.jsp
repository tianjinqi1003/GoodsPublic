...
<link rel="stylesheet" href="<%=basePath %>/resource/css/spzs/whiteWine/batchAddModule.css" />
...
function nextStep(flag){
	...
	if(stepIndex==1){
		...
		$("input[id^='spxqIfShow']").each(function(i){
			...
			if(checked=="true"){
				//console.log($("input[name^='spxqName']").eq(i).val());
...
function resetXzmbExcelTabStyle(){
...
function downloadExcelModule(){
	...
	jsonStr+="]";
	console.log(jsonStr);
	location.href=path+"merchant/excel/downloadExcelModule?trade=spzs&moduleType=whiteWine&jsonStr="+encodeURI(jsonStr);
}
