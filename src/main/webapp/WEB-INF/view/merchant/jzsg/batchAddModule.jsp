...
<title>编辑</title>
<%@include file="../js.jsp"%>
<link rel="stylesheet" href="<%=basePath %>/resource/css/jzsg/batchAddModule.css" />
...
function nextStep(flag){
	...
	if(stepIndex==1){
		...
		$("input[id^='ryxxIfShow']").each(function(i){
			...
			if(checked=="true"){
				txtTdStr+="<td>"+substringName($("input[name^='ryxxName']").eq(i).val())+"</td>";
				txtColIndex++;
			}
			else{
				noTxtTdStr+="<td></td>";
		...
		for(var i=1;i<=3;i++){
			...
			$("input[id^='ryxxIfShow']").each(function(j){
				...
				if(checked=="true"){
					if(txtColIndex==0)
						txtTdStr+="<td class=\"num_td\">"+substringName($("input[name^='ryxxName']").eq(j).val()+i)+"</td>";
					else
						txtTdStr+="<td>"+substringName($("input[name^='ryxxName']").eq(j).val()+i)+"</td>";
					txtColIndex++;
				}
				else{
					if(txtColIndex==0)
	...
	<div class="ryxx_div" id="ryxx_div">
		<table class="ryxx_tab" id="ryxx_tab">
			...
			<tr class="item_tr" id="tr${status.index }" height="50">
				<td class="name_td">
					<input type="text" name="ryxxName${status.index }" value="字段名未设置" size="10" maxlength="80" />
				</td>
				<td class="value_td">
					默认显示Excel导入内容
				</td>
